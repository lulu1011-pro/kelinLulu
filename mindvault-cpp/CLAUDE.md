...
- AI 增强 P2「图谱问答+function calling」已完成(2026-09-07):新增 graph_qa_service.h(关键词正则路由判定,结构化路纯SQL查link_edges不调AI,非结构化路关联笔记摘要喂LLM,ExtractNoteName从问题提取笔记名FTS5匹配);新增 function_calling_service.h(2个真实只读工具search_notes/get_note对应SearchService/NoteService,单轮tool loop,三层降级);新增 POST /api/ai/graph-qa + POST /api/ai/chat-with-tools 端点;前端 KnowledgeGraph.vue 加问答面板,AIChatPanel.vue 加工具模式开关。零新表零迁移,复用link_edges/notes/notes_fts。已修复ExtractNoteName死循环bug(remove_words含空格导致find总能命中)。
- P0-3.2 引用溯源已完成(2026-09-07):后端 /api/ai/chat(非流式)响应带 sources 字段,/api/ai/chat/stream 的 done 事件同步携带 sources(检索结果 id/title/folder/highlight);前端 AIChatPanel.vue 在 assistant 消息下渲染「📎 参考来源」chips,点击来源通过 select-note 事件跳转打开对应笔记;api.ts ChatMessage 增加可选 sources 字段、chatStream onDone 回调透传。另修复两个存量前端 bug:api.ts getRecommendations URL 模板字符串丢失反引号(请求路径错误)、BacklinksPanel.vue v-else-if 误接在 v-else 后(Vue 语法错误导致 vite build 失败)。
- 详细方案看 P0-1_DESIGN.md(已实现);P0/P1/P2 全量规划看 MindVault-AI功能增强设计_20260905.md(已更新为已实现状态,含第十三章P0批次+第十四章P1-5+第十五章P1-6/P1-7+第十六章P2已实现记录+API请求响应示例)。改 AI 功能时保持 /api/ai/chat 单轮调用兼容。新增端点 POST /api/ai/chat/stream(SSE流式)、POST /api/ai/action(内容创作统一接口)、GET /api/notes/:id/recommendations(关联推荐)、POST /api/ai/graph-qa(图谱问答)、POST /api/ai/chat-with-tools(function calling)。
